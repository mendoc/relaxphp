...
<div class="col-sm-8 d-none">
    <div class="title-action">
        <a href="<?= route('add') ?>"
           class="btn btn-primary d-print-none">Ajouter <?= heading('male') ? 'un' : 'une' ?> <?= heading('singular') ?></a>
    </div>
</div>
</div>
    ...
    <?php else : ?>
        <div class="middle-box text-center animated bounceIn">
            ...
            <div class="error-desc">
                Il n'y a pas encore de <?= heading('plural') ?> pour le moment. Vous pouvez en ajouter en choisissant un certificat en
                cliquant le bouton ci-dessous.<br/>
                <a href="<?= route('home', [], 'certificat') ?>"
                   class="btn btn-primary m-t"><i class="fa fa-file-o"></i> Choisir <?= heading('male', 'certificat') ? 'un' : 'une' ?> <?= heading('singular', 'certificat') ?></a>
            </div>
        </div>
    <?php endif; ?>
</div>